...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where agent_email='$email' AND status='READY TO CONTACT' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' ORDER BY id ASC");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where agent_email='$email' AND status='READY TO CONTACT' ORDER BY id ASC");   
    }
  }elseif($level!='Seller'){
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where  status='READY TO CONTACT' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' ORDER BY id ASC"); 
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where  status='READY TO CONTACT' ORDER BY id ASC");   
    }
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE agent_email='$email' AND status='READY TO CONTACT' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." ORDER BY id ASC");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE agent_email='$email' AND status='READY TO CONTACT' and 1 ".$searchQuery." ORDER BY id ASC");   
    }
   
  }elseif($level!='Seller'){
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE status='READY TO CONTACT' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." ORDER BY id ASC");   
    
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE status='READY TO CONTACT' and 1 ".$searchQuery." ORDER BY id ASC");   
    }
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $empQuery = "select * from joborders WHERE agent_email='$email' AND status='READY TO CONTACT' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;

    }else{
        $empQuery = "select * from joborders WHERE agent_email='$email' AND status='READY TO CONTACT' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
    }
    
}elseif($level!='Seller'){
    if ($to!='' && $from!='') {
        $empQuery = "select * from joborders WHERE status='READY TO CONTACT'and DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;    

    }else{
        $empQuery = "select * from joborders WHERE status='READY TO CONTACT' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;    
    }
