...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        // Not a valid email
        $error_msg .= '<p class="error">No es un correo valido.</p>';
    }
    ...
    if (strlen($password) != 128) {
        ...
        $error_msg .= '<p class="error">Mala configuracion de contraseña.</p>';
    }
    ...
    if ($stmt) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            $error_msg .= '<p class="error">Ya existe un usuario registrado con este correo.</p>';
        }
    } else {
...
else
{
  ...
  $invoice=$invoice+1;
    ...
    $uploadfile_temporal1=$_FILES['image1']['tmp_name']; 
    ...
    echo "error"; 
        ...
        $query1 = mysqli_query($connect, "INSERT INTO agents(name, phone, email, picture, level) VALUES ('$username','$phone', '$email', '$uploadfile_nombre1', '$level')") or die ('Error');
