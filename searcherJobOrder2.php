  ...
  <aside class="main-sidebar" style="background:#910007;">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li style="border-bottom:1px solid gray; padding:5px;">
            <span class="active" style="text-align:center; color:yellow; ">Quick Searcher <img src="./img/chinaFlag.png" style="width:30px; position:relative; top:-3px; left:10px;"></span>
  ...
  <div class="content-wrapper" >
    ...
    <section class="content">
      ...
      <div class="searchPage shadow2" style="background:white; width:90%; margin-left:-45%;">
        ...
        <h3 style="text-align:center; color:black; font-weight:400; font-size:20px; padding:30px; margin-top:-30px; border-bottom:1px solid black;">Searcher Job Order<br><br>

          Now Searching: "<span style="color:red; font-weight:bolder;"><?php echo $JO; ?></span>".
...
<style type="text/css">
 ...
 if ($level=='Seller' AND $JO!='') { 

                      if ($to!='' && $from!='') {

                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND fecha >= '$from' AND fecha < '$to' AND (customer_name LIKE '%$JO%' OR customer_company LIKE '%$JO%' OR supplier_name LIKE '%$JO%' OR supplier_company LIKE '%$JO%' OR id LIKE '%$JO%')   ORDER BY id asc ") or die ("Error al traer los datos"); 
                    }else{

                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND (customer_name LIKE '%$JO%' OR customer_company LIKE '%$JO%' OR supplier_name LIKE '%$JO%' OR supplier_company LIKE '%$JO%' OR id LIKE '%$JO%') ORDER BY id asc ") or die ("Error al traer los datos"); 

                    }



                    }elseif($level!='Seller' AND $JO!=''){

                      if ($to!='' && $from!='') {

                    $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND fecha >= '$from' AND fecha < '$to' AND (customer_name LIKE '%$JO%' OR customer_company LIKE '%$JO%' OR supplier_name LIKE '%$JO%' OR supplier_company LIKE '%$JO%' OR id LIKE '%$JO%')  ORDER BY id asc ") or die ("Error al traer los datos");
                      }else{
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND (customer_name LIKE '%$JO%' OR customer_company LIKE '%$JO%' OR supplier_name LIKE '%$JO%' OR supplier_company LIKE '%$JO%' OR id LIKE '%$JO%')  ORDER BY id asc ") or die ("Error al traer los datos");
                      }
...
<button style="font-size:11px; background:#E2E2E2; width:60px; position:relative; left:0px; color:black; border-radius:3px; border:none; height:18px; position:relative; top:13px; font-weight:600;">ADD WR</button></a>
                   </td>
