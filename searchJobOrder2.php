...
<head>
  ...
  <title>Latim Cargo & Trading | Search Job Order</title>
      ...
      <br><br>
        ...
        <li class="active treeview" style="border-bottom:1px solid gray; padding:5px;">
          ...
          <ul class="treeview-menu">
            ...
            <li ><a class="active" href="searchJobOrder.php" style="font-size:11px;"><i class="fa fa-circle-o"></i>Search</a></li>
          </ul>
        </li>
  ...
  <div class="content-wrapper" >
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Search Job Order</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="searchPage shadow2" style="background:white; width:90%; margin-left:-45%;">
        ...
        <h3 style="text-align:center; color:black; font-weight:400; font-size:20px; padding:30px; margin-top:-44px; border-bottom:1px solid black;">Search Job Order
          <br><br>


          <div style="padding:10px; border:0px solid black; width:650px; left:50%; margin-left:-325px; margin-top:-70px; position:relative; top:40px; height:35px; background:#B80008;">

            <a href="?status=quick" style="color:black;"><button style="width:200px; font-size:13px; position:relative; top:-6px;<?php if ($status=='quick'){ ?>background:#007F46;<?php }else{ ?> background:#B80008;<?php } ?> color:white; border:1.3px solid gray; padding:4px;">Quick Job Orders</button></a>

            <a href="?status=warehouse" style="color:black;"><button style="width:200px; font-size:13px; position:relative; top:-6px; <?php if ($status=='warehouse'){ ?>background:#007F46;<?php }else{ ?> background:#B80008;<?php } ?> color:white; border:1.3px solid gray; padding:4px;">In Warehouse Only</button></a>

            <a href="?type=all" style="color:black;"><button style="font-size:13px; padding:8px;">All Job Orders</button></a>

            <a href="excel.php" style="color:black;"><button style="font-size:13px; padding:8px;">Download EXCEL</button></a>
            ...
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th style="width:100px;">Ship To:</th>
                  <th>Agent</th>
                  <th>Status</th>
                  <th>WR #</th>
                  <th>Shorcuts</th>
                ...
                <tbody>
                  ...
                  <?php 
                    ...
                    if ($level=='Seller' && $type=='quick') { 
                      ...
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND fecha >= '$from' AND fecha < '$to' AND (status='PENDING' OR status='READY TO CONTACT' OR status='CHECK NOTES' OR status='IN PROCESS' OR status='SHIPPED' OR status='CANCELED' OR (status='IN WAREHOUSE' AND fecha > '2020-01-01') )    ORDER BY id asc ") or die ("Error al traer los datos"); 
                    }else{

                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND (status='PENDING' OR status='READY TO CONTACT' OR status='CHECK NOTES' OR status='IN PROCESS' OR status='SHIPPED' OR status='CANCELED' OR (status='IN WAREHOUSE' AND fecha > '2020-01-01') )  ORDER BY id asc ") or die ("Error al traer los datos"); 
                    }
                    ...
                    $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND fecha >= '$from' AND fecha < '$to'  ORDER BY id asc ") or die ("Error al traer los datos");
                      }else{
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' ORDER BY id asc ") or die ("Error al traer los datos");
                      }

          }elseif($level=='Seller' && $type=='warehouse') {
                      if ($to!='' && $from!='') {

                    $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND fecha >= '$from' AND fecha < '$to' AND (status='IN WAREHOUSE')  ORDER BY id asc ") or die ("Error al traer los datos");
                      }else{
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE agent_email='$email' AND branch='' AND (status='IN WAREHOUSE') ORDER BY id asc ") or die ("Error al traer los datos");
                      }
                    ...
                    $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND fecha >= '$from' AND fecha < '$to' AND (status='PENDING' OR status='READY TO CONTACT' OR status='CHECK NOTES' OR status='IN PROCESS' OR status='SHIPPED' OR status='CANCELED' OR (status='IN WAREHOUSE' AND fecha > '2020-01-01') )  ORDER BY id asc ") or die ("Error al traer los datos");
                      }else{
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND (status='PENDING' OR status='READY TO CONTACT' OR status='CHECK NOTES' OR status='IN PROCESS' OR status='SHIPPED' OR status='CANCELED' OR (status='IN WAREHOUSE' AND fecha > '2020-01-01') )   ORDER BY id asc ") or die ("Error al traer los datos");
                      }
                    ...
                    $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND fecha >= '$from' AND fecha < '$to'  ORDER BY id asc ") or die ("Error al traer los datos");
                      }else{
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' ORDER BY id asc ") or die ("Error al traer los datos");
                      }
                    ...
                    $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND fecha >= '$from' AND fecha < '$to' AND (status='IN WAREHOUSE')  ORDER BY id asc ") or die ("Error al traer los datos");
                      }else{
                      $consulta = mysqli_query($connect, "SELECT * FROM joborders WHERE branch='' AND (status='IN WAREHOUSE') ORDER BY id asc ") or die ("Error al traer los datos");
                      }
   ...
   date('d/m/y',$t); ?>
                <tr>
                  ...
                  <td style="width:100px;">
                    <a href="#" style=" text-align:center; position:relative;  top:16px; color:black; font-weight:600;">
                      <p style="font-size:12px; font-weight:600;"><?php echo date('Y-m-d <br>H:i:s',$t); ?></p></a>
                  </td>

                  <td>
                    <a href="#" style=" text-align:center; position:relative;  top:16px; color:black; font-weight:600;">
                      <p style="font-size:12px;"><?php echo $row['id']; ?></p></a>
                      <center><img src="./img/chinaFlag.png" style="width:30px; margin-top:10px;"></center>
                  </td>

                  <td><p style=" text-align:center; position:relative; font-size:12px; top:6px; color:black; font-weight:600;"><?php echo $customer_if; ?></p></td>


                  <td style="width:40px;"><span style="margin-left:15px; text-align:justify; position:relative; font-size:12px; top:6px; color:black; font-weight:600;"><?php echo $supplier_if; ?></span></td>
                  ...
                  <td>
                    ...
                    <p style=" text-align:center; font-size:10px; position:relative; top:16px; color:black; font-weight:600;">
                      <i style="font-size:25px;" class="icon fa <?php if ($row['service']=='Air door to door' OR $row['service']=='Air Service'){ ?>fa-plane<?php } ?> <?php if ($row['service']=='Ocean door to door' OR $row['service']=='Ocean Service'){ ?>fa-ship<?php } ?> <?php if ($row['service']=='Pending'){ ?>fa-hourglass-2<?php } ?> "></i><br>
    ...
    vertical-align: middle;">
                    ...
                    <span style="font-weight:bolder; font-size:12px; font-weight:600;"><?php echo $row['customer_city']; ?></span>
                    <br>
                      <?php if ($row['customer_country']=='VE'){ ?>
                        <img src="./img/venezuela.png" style="width:20px;">
                      <?php }elseif ($row['customer_country']=='CN') { ?>
                        <img src="./img/chinaFlag.png" style="width:20px;">
                     <?php } ?>
...
<button style="font-size:11px; background:#E2E2E2; width:60px; position:relative; left:8px; color:black; border-radius:3px; border:none; height:18px; position:relative; font-weight:600; top:13px;">ADD WR</button></a>
                   </td>
...
'left=20,top=20,width=900,height=700,toolbar=1,resizable=0'); return false;" style="cursor:pointer; background-color:rgb(0,0,0,0); position:relative; left:5px; color:#B80008; font-size:16px; border:none;">
                    ...
                    <?php $webPDF='warehouseEntry.php'; if ($row['agent_name']=='Saspy Express'){ $webPDF='downloadPDFsaspy.php'; } ?>
