...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where agent_email='$email' AND branch='USA' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' ORDER BY id ASC");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where agent_email='$email' AND branch='USA' ORDER BY id ASC");   
    }
  }elseif($level!='Seller'){
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where  branch='USA' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' ORDER BY id ASC"); 
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders where  branch='USA' ORDER BY id ASC");   
    }
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE agent_email='$email' AND branch='USA' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." ORDER BY id ASC");
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE agent_email='$email' AND branch='USA' and 1 ".$searchQuery." ORDER BY id ASC");   
    }
   
  }elseif($level!='Seller'){
    if ($to!='' && $from!='') {
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE branch='USA' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." ORDER BY id ASC");   
    
    }else{
        $sel = mysqli_query($connect,"select count(*) as allcount from joborders WHERE branch='USA' and 1 ".$searchQuery." ORDER BY id ASC");   
    }
...
if ($level=='Seller') { 
    if ($to!='' && $from!='') {
        $empQuery = "select * from joborders WHERE agent_email='$email' AND branch='USA' AND DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;

    }else{
        $empQuery = "select * from joborders WHERE agent_email='$email' AND branch='USA' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
    }
    
}elseif($level!='Seller'){
    if ($to!='' && $from!='') {
        $empQuery = "select * from joborders WHERE branch='USA'and DATE_FORMAT(`fecha`,'%Y-%m-%d') BETWEEN '$from' and '$to' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;    

    }else{
        $empQuery = "select * from joborders WHERE branch='USA' and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;    
    }
